...
def do_all(inputDict):
    ...
    if inputDict['njoy']:
        ...
        njoy.get_njoy_temperatures(globalZASabList, njoyTDict, globalTDict, globalTXSDict)
        njoy.get_njoy_background_xs(globalZASabList, njoyBXSDict, globalBXSDict, useCommonBXS)
        glob.print_globals(globalZASabList, globalZAList, globalZList, globalTDict,
            globalBXSDict, globalTXSDict, verbosity)
        njoy.print_njoys(njoyTDict, njoyBXSDict, verbosity)
        njoy.print_mcnp_material_inputs(materials, njoyTDict, verbosity)
        njoy.create_njoy_decks(inputDict, globalZASabList, njoyTDict, njoyBXSDict, globalTXSDict, verbosity)
...
def define_input_parser():
    ...
    bondarenkoGroup.add_argument('-p', '--printopt', help='Which XS to print. Usual prints selected XS and the combined transfer matrix; abs prints the usual in addition to some cross sections necessary to determine the total absorption rate; total just prints the flux and total XS; separate prints all XS and transfer matrices except the combined transfer matrix; all prints everything; none prints no PDT XS.', choices=['none', 'total', 'usual', 'abs', 'separate', 'all'], default='usual')
